...
export default function LeavePage() {
  ...
  useEffect(() => {
    async function getSession() {
      try {
        ...
        console.error('Failed to load leave management session:', err);
        router.push('/login');
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <AppLayout>
      {session.role === Role.EMPLOYEE && <EmployeeLeaveView session={session} />}
      {session.role === Role.MANAGER && <ManagerLeaveView session={session} />}
      {session.role === Role.HR_ADMIN && <HrLeaveView session={session} />}
    </AppLayout>
  );
}
