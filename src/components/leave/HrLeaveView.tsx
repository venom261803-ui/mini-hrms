...
interface HrLeaveViewProps {
  session: UserSession;
}

export default function HrLeaveView({ session }: HrLeaveViewProps) {
  ...
  // Fetch Company Leave Requests with Query Parameters
  const fetchAllLeave = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        setError(result.error || 'Failed to load company leave requests.');
        return;
      }

      setLeaveRequests(result.data?.leaveRequests || []);
    } catch (err: any) {
      setError(err.message || 'Network error fetching company leave requests.');
    } finally {
      setIsLoading(false);
    }
  }, [employeeIdFilter, statusFilter, leaveTypeFilter, startDateFilter, endDateFilter]);
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-xl border border-slate-800 bg-slate-900/90 shadow-lg">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white">Company-Wide Leave Management</h1>
          <p className="text-xs text-slate-400 mt-1">
            Global access to all company leave requests, filtering, approval queue, and audit history.
          </p>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-slate-700 bg-slate-950 p-2 text-xs text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/90 shadow-xl overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-white">Company Leave Directory</h3>
          </div>
        ...
        {isLoading && !error && (
          <div className="p-8 text-center text-slate-400 space-y-3">
            ...
            <p className="text-xs font-medium">Loading company leave requests...</p>
          </div>
        )}
        ...
        {!isLoading && !error && leaveRequests.length === 0 && (
          <div className="p-12 text-center text-slate-400 space-y-3">
            ...
            <p className="text-xs text-slate-500">
              There are no leave requests matching your current filter parameters.
            </p>
          </div>
        )}
            ...
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="bg-slate-950/80 text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-800">
                <tr>
                  ...
                  <th className="py-3 px-4">Reason / Reviewer</th>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <td className="py-3.5 px-4">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-blue-400 font-mono">{empCode}</span>
                          {dept && (
                      ...
                      <td className="py-3.5 px-4 max-w-xs">
                        ...
                        {item.reviewedBy && (
                          <span className="text-[11px] text-slate-400 block mt-1">
                            Reviewed by: <strong className="text-slate-300">{item.reviewedBy.fullName}</strong>
                          </span>
                        )}
